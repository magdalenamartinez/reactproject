...
function TengoCuenta() {
    ...
    useEffect(() => {
      ...
      const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch('/loginUserRoute/try-login', {method:'POST', headers: {'Content-Type': 'application/json',}, body: JSON.stringify({user, password, recaptchaToken: recaptchaToken, table:'clientes'}),});
            if (response.ok) {
