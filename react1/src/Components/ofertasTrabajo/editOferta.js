...
import useInitialOfertState from "../funcionalidades/initialForm/initialOfertstate.js";
import handleChange from "../profile/funciones/handleChange";
...
const handleSubmit = async (event) => {
      ...
      formData.append('p1', ofertaData.politicasInclusivas === 0? 1:0);
      formData.append('p2', ofertaData.instalacionesAccesibles === 0? 1:0);
      formData.append('p3', ofertaData.formacionInclusividad === 0? 1:0);
      formData.append('p4', ofertaData.mentoresApoyo === 0? 1:0);
      formData.append('p5', ofertaData.ambienteAdaptado === 0? 1:0);
      console.log( ofertaData.politicasInclusivas === 0? 1:0);
      formData.append('id_oferta', ofertaData.id);
      
      await updateData(formData, 'oferta_empleo-edit', navigate);
        ...
        <form className="form_class_big" id="form_id" onSubmit= {handleSubmit} action="/ofertaRoute/update-data-oferta" encType='multipart/form-data' method="post">
            <div className="bloque_form" id="empresa">
                ...
                <div className='comun'>
                   ...
                   <Foto onChange={(event) => {handleC(event); InputChange2('imageInput', 'imageShoww'); setDeleteImage(false);}} styleFoto={{display: ofertaData.image ? 'initial' : 'none'}}
                   handleDeleteImage={handleDeleteImage} src={ofertaData.image? `https://backend-empleoinclusivo.onrender.com/uploads/${ofertaData.image}`:'/images/uploadimage2.png'}
                   textFoto={'Foto de la Oferta'}/>
                </div>
            </div>
            ...
            &nbsp;
            <button className={`submit_button ${st.botonContrast}`} type="submit">Terminar Edición</button>
            &nbsp;
            <button className={`submit_button mini ${st.botonContrast}`} type="button" onClick={handleExit}>Cancelar Edición</button>
            <br/>
        </form>
    </div>
  </div>  
